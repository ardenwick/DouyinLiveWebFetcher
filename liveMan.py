...
class DouyinLiveWebFetcher:
    ...
    def get_room_status(self, retries=30) -> Tuple[dict, str, str]:
        ...
        for retry in range(retries):
            ...
            interval = (retry % 4) * 5 + 5
            logger.debug(f"Sleeping for {interval} seconds before retry.")
            time.sleep(interval)
    ...
    def _connectWebSocket(self):
        ...
        wss = ("wss://webcast100-ws-web-lq.douyin.com/webcast/im/push/v2/?app_name=douyin_web"
               ...
               "&screen_width=5120&screen_height=1440&browser_language=zh-CN&browser_platform=Win32"
        ...
        cookies = self.read_file_if_exist("./cookies.txt")
        if not cookies:
    ...
    def _gatherUsers(self, m: Message, method: str):
        ...
        if method == 'MemberMessage':
            self.user_db.updateMany([
                u for u in getMessageFields(m, paths) if u.pay_grade.level > 10
            ])
